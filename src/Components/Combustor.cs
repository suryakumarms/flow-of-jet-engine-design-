...
namespace JetEngine
{
    ...
    public static class Combustor
    {
        public static void BuildCombustor(CombustorDesign comb, float sc, BBox3 domain, string outDir, float zComb, float coreR)
        {
            ...
            JetEngineFabrication.SaveSTL(vCombustor, outDir, "Jet_Combustor.stl");
            ...
            var vInj = new Voxels();
            float injR2 = (combIR + combOR) / 2f;
            for (int i = 0; i < 12; i++)
            {
                float a2 = i * 30f * MathF.PI / 180f;
                float cx = injR2 * MathF.Cos(a2);
                float cy2 = injR2 * MathF.Sin(a2);
                var st2 = new Voxels(new SdfCylinder(new Vector3(cx * 1.2f, cy2 * 1.2f, zComb + 10f), new Vector3(cx * 0.7f, cy2 * 0.7f, zComb + 10f), 5f), domain);
                st2.BoolAdd(new Voxels(new SdfCylinder(new Vector3(cx * 0.7f, cy2 * 0.7f, zComb + 5f), new Vector3(cx * 0.7f, cy2 * 0.7f, zComb + 15f), 8f), domain));
                vInj.BoolAdd(st2);
            }
            JetEngineFabrication.SaveSTL(vInj, outDir, "Jet_Injectors.stl");
            Library.oViewer().Add(vInj, 11);
            Library.oViewer().SetGroupMaterial(11, new ColorFloat(0.9f, 0.5f, 0.2f), 0.9f, 0.05f);

            // 2x Igniter Plugs (radial cylinders at zComb + 15mm, at angles 45° and 135°)
            var vIgniters = new Voxels();
            float igniterRad = (combIR + combOR) / 2f;
            float rOuterCasingComb = combOR + 30f;
            for (int i = 0; i < 2; i++)
            {
            ...
            JetEngineFabrication.SaveSTL(vIgniters, outDir, "Jet_Igniters.stl");
            ...
            JetEngineFabrication.SaveSTL(vSnout, outDir, "Jet_Combustor_Snout.stl");
