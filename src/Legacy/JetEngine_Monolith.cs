...
namespace JetEngine
{
    ...
    public static class RotorDynamics
    {
                                                ...
                                                double shaftOD, double shaftID, double totalMass)
        {
            ...
            double omega1 = Math.Pow(Math.PI / shaftLength, 2) * Math.Sqrt(E * I / rhoA);
            double omega2 = 4.0 * omega1;  // Second mode
            ...
            bool passed = margin1 > 15.0 && margin2 > 15.0;
            
            Console.WriteLine($"    ω_cr1={crit1:F0}RPM  ω_cr2={crit2:F0}RPM  " +
                              $"Operating={rpm:F0}RPM  Margins={margin1:F1}%,{margin2:F1}%  " +
                              $"{(passed?"✓":"✗ WHIRL RISK")}");
    ...
    public static class ShaftMechanicals
    {
            ...
            CycleResult cycle)
        {
            ...
            // Compressor stages — pressure rise → forward force on disc
            foreach (var s in compressors)
            {
                double A_ann = Math.PI * (s.TipRadius * s.TipRadius - s.HubRadius * s.HubRadius);
                // Pressure inlet/exit (station-level): use temperature ratio to estimate
                double dP_stage = cycle.CoreMassFlow > 0
                    ? (s.Temperature_Out - s.Temperature_In) / s.Temperature_In
                      * cycle.Stations.Values.First().Pt * 0.1  // rough: 10% of inlet Pt per stage
                    : 0;
                // Momentum: ṁ·(Vz_in - Vz_out) ≈ 0 for axial compressor (Va ≈ const)
                // Main contribution is disc pressure loading
                r.CompressorForce_N += Math.Abs(dP_stage) * A_ann;
            }

            // Turbine stages — pressure drop → rearward force on disc
            foreach (var s in turbines)
            {
                double A_ann = Math.PI * (s.TipRadius * s.TipRadius - s.HubRadius * s.HubRadius);
                double dP_stage = cycle.CoreMassFlow > 0
                    ? (s.Temperature_In - s.Temperature_Out) / s.Temperature_In
                      * cycle.Stations.Values.First().Pt * 0.08
                    : 0;
                r.TurbineForce_N += Math.Abs(dP_stage) * A_ann;
            }
    ...
    public static class JetEngineFabrication
    {
        ...
        static void Generate(CycleResult cycle, EngineFlowPath fp, CombustorDesign comb)
        {
            ...
            var vCasing = new Voxels(new SdfRevolution(
