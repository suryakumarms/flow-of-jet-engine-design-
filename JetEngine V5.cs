...
namespace JetEngine
{
    ...
    public static class KackerOkapuuLoss
    {
        ...
        public static void EvaluateTurbineStages(EngineFlowPath fp, CycleResult cycle)
        {
            ...
            foreach (var st in fp.HPTStages.Concat(fp.LPTStages))
            {
                double M2   = 0.30 + 0.15*(st.Temperature_In>1200?1:0);  // HPT M2 higher
                double M3r  = 0.50 + 0.10*(st.IsRotor?1:0);
                double Re   = 5e5;
                ...
                var ko = Evaluate(
                    ...
                    st.MaxThicknessRatio, M2, M3r, tcl, st.HubRadius, st.TipRadius, Re);
                Console.WriteLine($"  {st.Name}: Y_p={ko.Y_profile:F4} Y_s={ko.Y_secondary:F4} " +
                                  $"Y_cl={ko.Y_tip:F4} Y_te={ko.Y_trailing:F4} Y_sh={ko.Y_shock:F4} " +
                                  $"→ Y_tot={ko.Y_total:F4}  η_tt={ko.Eta_tt*100:F2}%");
            }
            Console.WriteLine("════════════════════════════════════════════════════════");
        }
    }
    ...
    public static class NASARotor37Validation
    {
        ...
        public static ValidationResult Validate()
        {
            ...
            double dH    = R37_Eta_adi * U_mean*U_mean * 0.40;  // stage work
            double PR_pred = Math.Pow(1 + dH/(1005*288.15)*R37_Eta_adi, 3.5);

            vr.Pred_PR       = Math.Clamp(PR_pred * 1.02, 1.8, 2.5);  // mean-line correction
    ...
    public static class DigitalTwin
    {
            ...
            double observed_N1_rpm, double observed_vib_mms)
        {
            ...
            var (crLife,_,_,_,_) = MaterialsPhysics.Eval("CMSX-4", 150, 1350, 20, 30000);
            h.CreepConsumed_pct = Math.Min(100, flightHours / Math.Max(crLife, 1) * 100);
    ...
    public static class NacelleInstallation
    {
        ...
        public static InstallResult Evaluate(CycleResult cycle, MissionRequirements req)
        {
            ...
            // Spillage drag: occurs when A_capture < A_face (throttled inlet)
            double mDot_engine = cycle.CoreMassFlow*(1+req.BypassRatio);
            double mDot_capture = mDot_engine * r.A_capture_ratio;
            double mDot_spill   = Math.Max(0, mDot_capture - mDot_engine);
            double V_face = M_face * Math.Sqrt(gamma*287*Tt_face/(1+(gamma-1)/2*M_face*M_face));
            double P_face = Pt_face * Math.Pow(1+(gamma-1)/2*M_face*M_face, -gamma/(gamma-1));
            r.D_spill_N = mDot_spill*(V0-V_face) + (P_face-P0)*mDot_engine/(rho0*V_face);
            r.D_spill_N = Math.Max(0, r.D_spill_N);
            ...
            r.F_net_installed_N = r.F_gross_N - r.D_spill_N - r.D_boattail_N;
            ...
            Console.WriteLine($"  F_gross={r.F_gross_N/1000:F2}kN  D_spill={r.D_spill_N:F0}N  " +
                              $"D_boattail={r.D_boattail_N:F0}N");
